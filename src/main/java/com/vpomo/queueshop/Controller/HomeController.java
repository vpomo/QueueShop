...
return "index";    
    ...
    public String ErrorTest(Principal principal, Model model) {
        return "/error";
    }
...
queueCashbox1.clear();queueCashbox1.trimToSize();
queueCashbox2.clear();queueCashbox2.trimToSize();
queueCashbox3.clear();queueCashbox3.trimToSize();
queueCashbox4.clear();queueCashbox4.trimToSize();

PurchaserFactory.getPurchaserInstance().getPurchaserDAO().clearAll();
...
for (int step = 1; step < numSteps+1; step++) {
    ...
    if (quantityGoods%powerCashbox > 0) {waitCashbox=waitCashbox+1;}
...
currentPurchaser = PurchaserFactory.getPurchaserInstance().getPurchaserDAO().CreatePurchaser(typePurchaser,cashBox,quantityGoods,waitCashbox-1); 
switch(cashBox) {
        case 1: 
	    queueCashbox1.add(currentPurchaser);
            PurchaserFactory.getPurchaserInstance().getPurchaserDAO().UpdateStepPurchaser(currentPurchaser, step, 1000);
            break;
        case 2: 
...
logger.info("AAAAAAAAAAAAAAAAAAAA=" + currentPurchaser.getId());
...
switch(cashbox) {
        case 1: 
            ...
            if(lengthQueue1>0) {
                ...
                if (waitQueue1 > 0){queueCashbox1.get(0).setWaitCashbox(waitQueue1-1);}

                for(int j = 0; j < lengthQueue1; j++) {
                logger.info(queueCashbox1.get(j).toString() + "  j=" + j);
                PurchaserFactory.getPurchaserInstance().getPurchaserDAO().UpdateStepPurchaser(queueCashbox1.get(j), currentStep, j+1);
                }

                if (waitQueue1 == 0){ pur = queueCashbox1.remove(0); }        
	...
	break;
        case 2: 
            ...
            if(lengthQueue2>0) {
                ...
                if (waitQueue2 > 0){queueCashbox2.get(0).setWaitCashbox(waitQueue2-1);}

                for(int j = 0; j < lengthQueue2; j++) {
                logger.info(queueCashbox2.get(j).toString());
                PurchaserFactory.getPurchaserInstance().getPurchaserDAO().UpdateStepPurchaser(queueCashbox2.get(j), currentStep, j+1);
                }    
                
                if (waitQueue2 == 0){ pur = queueCashbox2.remove(0); }        
            }            
            break;
        case 3: 
            ...
            if(lengthQueue3>0) {
                ...
                if (waitQueue3 > 0){queueCashbox3.get(0).setWaitCashbox(waitQueue3-1);}

                for(int j = 0; j < lengthQueue3; j++) {
                logger.info(queueCashbox3.get(j).toString());
                PurchaserFactory.getPurchaserInstance().getPurchaserDAO().UpdateStepPurchaser(queueCashbox3.get(j), currentStep, j+1);
                }    

                if (waitQueue3 == 0){ pur = queueCashbox3.remove(0); }        
	...
	break;
        case 4: 
            ...
            if(lengthQueue4>0) {
                ...
                if (waitQueue4 > 0){queueCashbox4.get(0).setWaitCashbox(waitQueue4-1);}

                for(int j = 0; j < queueCashbox4.size(); j++) {
                logger.info(queueCashbox4.get(j).toString());
                PurchaserFactory.getPurchaserInstance().getPurchaserDAO().UpdateStepPurchaser(queueCashbox4.get(j), currentStep, j+1);
                }    

                if (waitQueue4 == 0){ pur = queueCashbox4.remove(0); }        
            }            
	break;
        default: 
	    //оператор;
...
logger.info("ManNumberQueue=" + numberQueue);   
logger.info(" sumWaitCashbox1=" + sumWaitCashbox1 +" sumWaitCashbox2=" + sumWaitCashbox2 +" sumWaitCashbox3=" + sumWaitCashbox3 +" sumWaitCashbox4=" + sumWaitCashbox4); 
return numberQueue;
}
...
logger.info("WomanNumberQueue=" + numberQueue);  
logger.info(" lengthQueue1=" + lengthQueue1 +" lengthQueue2=" + lengthQueue2 +" lengthQueue3=" + lengthQueue3 +" lengthQueue4=" + lengthQueue4);  
...
private Integer ChildAnalysisQueue() {
    ...
    logger.info("ChildNumberQueue=" + numberQueue);
return numberQueue;
}
